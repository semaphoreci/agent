...
type DockerComposeExecutor struct {
	...
	dockerConfiguration       api.Docker
...
func NewDockerComposeExecutor(dockerConfiguration api.Docker) *DockerComposeExecutor {
...
func (e *DockerComposeExecutor) Prepare() int {
	...
	for _, range e.dockerConfiguration.Services {
		dockerCompose += "  services:\n"
	}

	cmd := fmt.Sprintf("docker-compose -f %s run bash", e.dockerComposeManifestPath)

	e.terminal = exec.Command("docker-compose -f /tmpbash")
